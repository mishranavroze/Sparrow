...
def test_compile_text_includes_segment_structure():
    digest = _make_digest()
    text, segment_counts, _, _ = _compile_text(digest, "February 17, 2026")
    assert "# Podcast" in text  # generic fallback when no show is provided
...
def test_compile_text_includes_preamble():
    digest = _make_digest()
    text, _, _, _ = _compile_text(digest, "February 17, 2026")
...
def test_compile_text_segment_counts():
    ...
    digest = _make_digest(articles)
    _, segment_counts, _, _ = _compile_text(digest, "February 17, 2026")
...
def test_compile_text_truncates_long_content():
    ...
    text, _, _, _ = _compile_text(digest, "Feb 17, 2026")
